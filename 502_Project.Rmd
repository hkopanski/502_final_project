...
It was noted that the smaller difference between the alternative and null mean values were, the less likely the null hypothesis was rejected. Larger values of variance also reduced the number of rejected null hypothesis. In both cases, the more overlap between the two distributions, the less powerful the test. One way that can increase the power of the t-test is to increase the sample size. For a specific example, we can look at test cases #12 and #27. In both cases the distribution means were 6 and 5 for the null and alternative hypotheses respectively, along with variances of 1 for both distributions. The only difference between the two cases was the sample size ($n_{12}$ = 10 and $n_{27}$ = 100). This increase in sample size resulted in almost a 2 fold increase in power (570 vs 1000 rejected nulls). It stands to reason that large variances and small deltas in mean can be mitigated by increasing the sample size accordingly. 
...
|Test#|$\mu_{0}$|$\sigma_{0}^{2}$|$n_{0}$|$\mu_{a}$|$\sigma_{a}^{2}$|$n_{a}$|Test Results|
|-|-|-|-|-|-|-|-|
...
|7|5|4|10|5|1|10|`r sprintf("Test Results 7: %i", sum(test_results_7[,8]))`|
|8|5|9|10|5|1|10|`r sprintf("Test Results 8: %i", sum(test_results_8[,8]))`|
|9|6|1|10|5|1|10|`r sprintf("Test Results 9: %i", sum(test_results_9[,8]))`|
|10|6|4|10|5|1|10|`r sprintf("Test Results 10: %i", sum(test_results_10[,8]))`|
|11|6|9|10|5|1|10|`r sprintf("Test Results 11: %i", sum(test_results_11[,8]))`|
...
|31|6|4|70|5|1|100|`r sprintf("Test Results 31: %i", sum(test_results_31[,8]))`|
|32|6|4|100|5|1|100|`r sprintf("Test Results 32: %i", sum(test_results_32[,8]))`|
|33|6|9|70|5|1|100|`r sprintf("Test Results 33: %i", sum(test_results_33[,8]))`|
|34|6|9|100|5|1|100|`r sprintf("Test Results 34: %i", sum(test_results_34[,8]))`|
