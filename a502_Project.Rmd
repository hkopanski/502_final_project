...
In order to begin we must first find the difference in mean blood pressure between smoker and non smokers in this dataset. This was calculated to be `r (mu_smoker - mu_nonsmoker)`. Despite this being a non zero number we ar not sure if there is in fact a difference of that this value was due to chance.

In the first analysis we will be assuming equal variances, thereby allowing us to use pooled sample variance (see Appendix I: Equation 1) which was calculated to be `r round(p_sample_var_1, 1)` using a degree of freedom value `r dof_1`. Under these conditions the p value was calculated to be `r round(p_value_obs_1, 4)` which is smaller than $\alpha$ of 0.05 (0.025 for each side of the distribution). In terms of t values, our observed t value of `r round(t_obs_1, 2)` is smaller than the t value, `r round(t_stat_1, 2)` for a two sided $\alpha$ of 0.05.

Additionally, the 95% confidence limits were calculated for the pooled variance assumption. The observed 95% confidence intervals are `r CI_pooled[1]` to `r CI_pooled[2]`  It can be seen that 0 does not fall into the 95% confidence interval in this case. Therefore the null hypothesis can be rejected. 
